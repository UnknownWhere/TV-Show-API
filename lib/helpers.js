...
var trakt = new Trakt('bd1f2f72a4cdb58d66a267f89f0ebe1ed193c2f9aa7b1fece444e69f6cac70bb')
		...
		eztv.getAllEpisodes(show, function(err, data) {
			...
			if (!dateBased) {
				...
				try {
					...
					trakt.show(imdb,{extended:'full'}, function(err, show) {
				...
				if (err || !doc) {
					...
					try {
						trakt.show(slug, {extended:'full,images'},function(err, data) {
							if (!err && data) {
								console.log("satu");
								console.log("New Trakt Show" + JSON.stringify(data));
								...
								data.images.lowres = helpers.resizeImage(data.images.poster.full, 300)
								data.images.fanart = helpers.resizeImage(data.images.fanart.full, 940);
