...
var LATEST  =   "/tv/";
var SEARCH  =   "/usearch/";
var API_URL_HOURLY = BASE_URL + '/hourlydump.txt.gz';
var API_URL_DAILY= BASE_URL + '/dailydump.txt.gz';
...
kickass_extras.push({show: 'Mr Robot', slug: 'mr-robot-tv42422', provider: 'kickass'});
...
exports.getAllShows =   function(cb) {
	...
	console.log('Kickass URL : ' + URL);
	request(URL, function(err, res, html){
		console.log('Error Get Kickass' + html);
		if(err) { 
			console.log('Error Get Kickass' + err);
			return (err, null);
		}
		...
		allShows = kickass_extras.concat(allShows);
			...
			function (n, next) {
				...
				request.get(BASE_URL + "/" + data.slug +"/torrents/?page="+ p, function (err, res, html) {
					...
					show_rows.each(function() {
						...
						var seed = entry.parent().children('.green').text();
						var leecher = entry.parent().children('.red').text();
						...
						if(matcher) {
							...
							if(season && episode) {
								...
								var torrent = {};
								torrent.url = magnet;
								torrent.seeds = parseInt(seed);
								torrent.peers = parseInt(leecher);
...
exports.update = function(since, callback) {
	...
	var req = request(API_URL_DAILY, {encoding: null});
